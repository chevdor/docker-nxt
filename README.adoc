...
* First stop the first container (old version)
...
NOTE: When upgrading to a new version, the upgrade may take quite a while (like several long minutes until all the tables are migrated). Be patient! The NRS client will only be available once the update is finished.